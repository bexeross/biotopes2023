...
```
# Save the dataset
write the full dataset to csv with sys.Date appended to keep track of which is the active sample that day
```{r}
## export whole dataset (i.e., without subsetting for manageability)
write.csv(sample_info_f, file=file.path(dataPath,(paste0("biotopeInputSamples_", Sys.Date(),".csv"))))

```

# Subset samples for maneagbility
## Take a random (sub)sample from each Ref station
If necessary take a subset of samples then write the subsample to csv with sys.Date appended to keep track of which is the active sample that day.
...
# write.csv(sample_info_f_ss, file=file.path(dataPath,(paste0("biotopeInputSubsample_", Sys.Date(),".csv"))))
